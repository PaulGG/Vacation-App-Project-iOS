    ...
    @IBAction func done(_ sender: Any) {
        if origin.text != nil && destination.text != nil && provider.text != nil {
            ...
            if interval > 0 {
                ...
                flightModel.updateFlight(at: index!, toDest: toDest!, date: "\(String(dateComponents.month!))/\(String(dateComponents.day!))/\(String(dateComponents.year!))",  duration: Int(round(interval)), flyingFrom: origin.text!, flyingTo: destination.text!, gate: provider.text!, flightTime: "\(hour):\(minute)")
